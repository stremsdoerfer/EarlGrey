...
@implementation NSURLConnection (GREYApp)
...
+ (void)load {
  ...
  BOOL swizzleSuccess =
      [swizzler swizzleClass:self replaceClassMethod:originalSelector withMethod:swizzledSelector];
...
- (void)grey_trackPending {
  ...
  objc_setAssociatedObject(self, @selector(grey_trackPending), object,
                           OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)grey_untrackPending {
  GREYAppStateTrackerObject *object = objc_getAssociatedObject(self, @selector(grey_trackPending));
...
+ (NSData *)greyswizzled_sendSynchronousRequest:(NSURLRequest *)request
  ...
  NSData *data = INVOKE_ORIGINAL_IMP3(
      NSData *, @selector(greyswizzled_sendSynchronousRequest:returningResponse:error:), request,
      response, error);
  if (object) {
    UNTRACK_STATE_FOR_OBJECT(kGREYPendingNetworkRequest, object);
...
+ (void)greyswizzled_sendAsynchronousRequest:(NSURLRequest *)request
  ...
  INVOKE_ORIGINAL_IMP3(void,
                       ...
                       request, queue, completionHandler);
}
...
+ (NSURLConnection *)greyswizzled_connectionWithRequest:(NSURLRequest *)request
  ...
  NSURLConnection *connection = INVOKE_ORIGINAL_IMP2(
      NSURLConnection *, @selector(greyswizzled_connectionWithRequest:delegate:), request,
      proxyDelegate);
  return connection;
}
...
- (instancetype)greyswizzled_initWithRequest:(NSURLRequest *)request
  ...
  GREYNSURLConnectionDelegate *proxyDelegate =
      [[GREYNSURLConnectionDelegate alloc] initWithOriginalNSURLConnectionDelegate:delegate];
  id instance = INVOKE_ORIGINAL_IMP2(id, @selector(greyswizzled_initWithRequest:delegate:), request,
                                     proxyDelegate);
...
- (instancetype)greyswizzled_initWithRequest:(NSURLRequest *)request
  ...
  return INVOKE_ORIGINAL_IMP3(id,
                              ...
                              request, proxyDelegate, startImmediately);
}
