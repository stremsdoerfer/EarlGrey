...
+ (void)load {
  ...
  GREYFatalAssertWithMessage(swizzleSuccess, @"Cannot swizzle UIViewController viewWillDisappear");
...
- (void)grey_trackAsRootViewControllerForWindow:(UIWindow *)window {
  ...
  if (!window || window.hidden) {
    ...
    objc_setAssociatedObject(self, @selector(greyswizzled_viewWillAppear:), object,
...
- (void)greyswizzled_viewWillMoveToWindow:(id)window {
  objc_setAssociatedObject(self, @selector(grey_isMovingToNilWindow),
                           (window == nil) ? @(YES) : @(NO), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)greyswizzled_viewDidMoveToWindow:(id)window
  ...
  INVOKE_ORIGINAL_IMP2(void, @selector(greyswizzled_viewDidMoveToWindow:shouldAppearOrDisappear:),
                       window, appearOrDisappear);
}
