...
- (BOOL)isIdleNow {
  GREYAppState stateToCheck =
      [self currentState] & (~GREY_CONFIG_UINTEGER(kGREYConfigKeyIgnoreAppStates));
  return stateToCheck == kGREYIdle;
}
    ...
    @autoreleasepool {
      ...
      if (appStateTrackerObjectInternal) {
        appStateTrackerObjectExternal =
            objc_getAssociatedObject(appStateTrackerObjectInternal, @selector(currentState));
      ...
      BOOL externalObjectExistsAlready =
          [self->_externalTrackerObjects containsObject:appStateTrackerObjectExternal];
      ...
      if (operation == kGREYClearState) {
        ...
        newState = track ? (originalState | state) : (originalState & ~state);
      }
      ...
      if (newState == kGREYIdle) {
        [self->_externalTrackerObjects removeObject:appStateTrackerObjectExternal];
        if (!appStateTrackerObjectInternal) {
        ...
        if (appStateTrackerObjectInternal) {
          ...
          objc_setAssociatedObject(appStateTrackerObjectInternal, @selector(currentState), nil,
                                   OBJC_ASSOCIATION_ASSIGN);
        }
      } else {
        [self->_externalTrackerObjects addObject:appStateTrackerObjectExternal];
        if (appStateTrackerObjectInternal) {
          objc_setAssociatedObject(appStateTrackerObjectInternal, @selector(currentState),
                                   appStateTrackerObjectExternal,
...
- (void)grey_clearState {
  [self grey_performBlockInCriticalSection:^id {
    ...
    for (GREYAppStateTrackerObject *externalObject in self->_externalTrackerObjects) {
      GREYObjectDeallocationTracker *internalObject = externalObject.object;
      objc_setAssociatedObject(internalObject, @selector(currentState), nil,
                               OBJC_ASSOCIATION_ASSIGN);
    }
    [self->_externalTrackerObjects removeAllObjects];
    return nil;
  }];
}
