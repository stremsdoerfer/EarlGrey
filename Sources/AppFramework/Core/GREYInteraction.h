...
- (instancetype)usingSearchAction:(id<GREYAction>)action
             onElementWithMatcher:(id<GREYMatcher>)matcher NS_REFINED_FOR_SWIFT;

/**
 * Performs an @c action on the selected UI element. Will crash if the action fails.
 *
 ...
 *
 * @param action     The action to be performed on the @c element.
 * @param[out] error Error populated on failure (passing nil may cause a crash).
 *
...
/**
 * Performs an @c assertion on the selected UI element. Will crash if the @c assertion fails.
 *
